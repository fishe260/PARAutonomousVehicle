...
public class AutonomousMCSerial implements Runnable{
	...
	public AutonomousMCSerial(){
		...
		for(SerialPort i : ports)
			if(i.getSystemPortName().equals("COM3") || i.getSystemPortName().equals("COM4"))
				telemetryPort = i;
		...
		try {
			while(!exitThread) { //Run this thread until program closes
				...
				if(!RFTActive) {
					...
					//AutonomousMCGUI.kartPacket[4] = xAccel; //Voltage
					//AutonomousMCGUI.kartPacket[5] = yAccel; //Current
	...
	public static boolean getRFTStatus() {
		return RFTActive;
	}	
