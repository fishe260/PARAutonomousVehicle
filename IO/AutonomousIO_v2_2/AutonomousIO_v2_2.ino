...
#include "printf.h"
#include <Wire.h>
//STEERING MOTOR CONTROLLER LIBRARY
#include <Servo.h>
//COMPASS LIBRARIES
...
#define ticksPerRev 4105  //CHANGE **IGNORE THIS**
#define LOWER_LIMIT -1150   //DEFINES HOW FAR CLOCKWISE/COUNTERCLOCKWISE THE STEERING GOES
...
void loop() {
  //UNCOMMENT THESE FUNCTIONS AS YOU IMPLEMENT MORE FUNCTIONALITY
  getRFCdata();
  //getIMUdata();
  //readGPS();
  //readCompass();
  ...
  if (mode) {   //If in auto mode, use the desired values from the RPi and not the remote
    desiredSteeringAngle = desiredSteeringAngleRPI;
    //desiredSpeed = OutputThrottleRPI;
  }
  
  if (!encoderStatus || !commRadioStatus) {   //If off-nominal, go to neutral position
    desiredSpeed = 1500;
