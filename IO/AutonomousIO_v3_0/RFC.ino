...
byte lastMode; //Keeping track of lastModes
uint32_t sticksUpStartTime; //See how long the sticks are up
bool sticksUpStart, sticksUpComplete, changeMode; //Sticks have been held up for sufficient time
...
void getRFCdata() {
  ...
  if (programMode != 7) {
    if (ch[4] > 1195 && ch[4] < 1205) { //RC
      ...
      desiredSteeringAngle = map(ch[3], 1000, 2000, 0, 255);
    }
